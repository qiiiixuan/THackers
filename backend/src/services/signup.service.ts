...
/**
 * Approve a signup (staff or caregiver approval)
 */
export const approveSignup = async (
  ...
  if (signup.status === SignupStatus.CANCELLED || signup.status === SignupStatus.DECLINED) {
...
export const checkInSignup = async (
  ...
  if (signup.status !== SignupStatus.APPROVED && signup.status !== SignupStatus.CHECKED_IN) {
...
/**
 * Check in by event check-in token (self check-in)
 */
